...
async def test_execute_fetches_credential_urls_locally(monkeypatch) -> None:
    ...
    # FORK: mock _fetch_raw (curl_cffi → scrapling → httpx tiered fetcher) instead
    # of upstream's httpx.AsyncClient pre-fetch. Fork's 3-tier fetcher handles the
    # fetch; credential URLs are already blocked from Jina by _url_carries_credentials().
