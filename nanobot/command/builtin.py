...
def build_help_text() -> str:
    ...
    # FORK: additional custom commands
    lines.append("/s — Show bot status (alias)")
    lines.append("/c — Clear session instantly (no memory saved)")
    lines.append("/rerun — Run rerun.bat in workspace")
    return "\n".join(lines)
...
def register_builtin_commands(router: CommandRouter) -> None:
    ...
    router.priority("/s", cmd_status)
    router.exact("/new", cmd_new)
    router.exact("/compact", cmd_compact)
    router.exact("/status", cmd_status)
    router.exact("/s", cmd_status)
